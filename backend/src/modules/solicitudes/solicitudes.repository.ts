...
export class SolicitudesRepository {
  ...
  listar(ctx: SpContext, filtros: Record<string, unknown>, page: number, pageSize: number): Promise<{ data: unknown[]; meta?: Record<string, unknown> }> {
    return this.sp.callCtxPaginado<unknown>("app.fn_solicitud_listar", ctx, [filtrosArg(filtros), page, pageSize]);
  }
  ...
  crear(ctx: SpContext, d: CrearSolicitudDto) {
    return this.sp.callCtx<Record<string, unknown>>("app.sp_solicitud_crear", ctx, [
      d.titulo, d.descripcion, d.lugar, d.areaId,
      d.impactoOperativoId ?? null, d.impactoComentario ?? null,
      d.prioridadPercibida ?? null, d.enviar ?? true,
    ]);
  }
  ...
  decidir(ctx: SpContext, id: string, d: DecidirSolicitudDto) {
    return this.sp.callCtx<Record<string, unknown>>("app.sp_solicitud_decidir", ctx, [
      id, d.tipo, d.comentario ?? null, d.motivoId ?? null,
      d.destinatarioId ?? null, d.solicitudPrincipalId ?? null,
    ]);
  }
}
