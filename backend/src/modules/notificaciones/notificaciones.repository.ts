...
export class NotificacionesRepository {
  ...
  listar(ctx: SpContext, soloNoLeidas: boolean, limite: number): Promise<{ data: unknown[]; meta?: Record<string, unknown> }> {
    return this.sp.callCtxPaginado<unknown>("app.fn_notificacion_listar", ctx, [soloNoLeidas, limite]);
  }
  marcarLeida(ctx: SpContext, id?: string) {
    return this.sp.callCtx<{ marcadas: number }>("app.sp_notificacion_marcar_leida", ctx, [id ?? null]);
  }
  pendientesCorreo(ctx: SpContext, limite: number) {
    return this.sp.callCtx<Array<Record<string, unknown>>>("app.fn_notificacion_pendientes_correo", ctx, [limite]);
  }
